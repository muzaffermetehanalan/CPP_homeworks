...
CPU::CPU(string Option){ /*Constructor sets Registers,PC and Option */
	setRegisters(); /* calls the setRegisters function */
	setPC(); /* calls the setPc function */
	setOption(Option);/* calls the setOption function */
}
void CPU::setOption(string Option)/*This function sets option */
{
	opt=Option;
}
...
void CPU::execute(string lineString){/*This function executes given string */
	if(isThatHalt(lineString)) /*If string is "HLT" */
	{
		print(); /* calls print function */
		if(opt[0]=='1') /*If option is 1 */
		{
	...
	else 
	{
		...
		if(isThatMov(lineString)) /* If string is MOV */
		{
			...
			while(lineString[i]==' ')/* Ignores ' ' characters */
				++i;
			if(secondRegConvert(lineString,i,indexArray))/*converts the regist*/
		...
		else if(isThatJmp(lineString))/* If string is JMP*/
		{
			while(lineString[i]==' ')/* Ignores ' ' characters */
				++i;
			if(secondRegConvert(lineString,i,indexArray))/*converts the register*/
			{
				++i;
		...
		else if(isThatPrn(lineString))/*If string is PRN */
		{
			while(lineString[i]==' ')/* Ignores ' ' characters */
				++i;
			if(secondRegConvert(lineString,i,indexArray))
...
void CPU::firstRegConvert(string lineString,int i,int indexArray[])
{
